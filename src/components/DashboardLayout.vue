<template>
  <div>
    ...
    <header
      class="fixed top-0 left-64 right-0 h-20 bg-white border-b border-gray-100 flex items-center justify-end px-20 shadow-md z-40">
      ...
      <div class="absolute left-8 flex items-center gap-2">
        ...
        <span class="text-lg  text-black">{{ pageTitle }}</span>
      </div>
      ...
      <div class="flex items-center gap-1 -mr-16 ">
        <button
          ...
          <div class="hidden sm:flex flex-col items-start ">
            <span class="text-sm font-medium">Menna Ahmed</span><span class="text-xs ">Admin</span>
          </div>
          <div class="ml-4">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
              class="lucide lucide-chevron-down h-4 w-4 hidden sm:block">
              <path d="m6 9 6 6 6-6"></path>
            </svg>
          </div>
        </button>
    ...
    <main
      :class="`flex-1 p-8 transition-all duration-300 overflow-x-hidden pt-24 ${sidebarCollapsed ? 'ml-16' : 'ml-64'}`">
      <router-view />
    </main>
  </div>
</template>
